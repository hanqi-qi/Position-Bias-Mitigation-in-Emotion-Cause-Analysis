...
def senEncode_softmax(s_senEncode, w_varible, b_varible, n_feature, doc_len):
    ...
    s = tf.nn.dropout(s, keep_prob=0.6)
    w = get_weight_varible(w_varible, [n_feature, 2])
    b = get_weight_varible(b_varible, [2])
    pred = tf.matmul(s, w) + b
    pred *= getmask(doc_len, 75, [-1, 1])
    pred = tf.nn.softmax(pred)
    pred = tf.reshape(pred, [-1, 75, 2])
    reg = tf.nn.l2_loss(w) + tf.nn.l2_loss(b)   # reg是正则项
    return pred, reg
